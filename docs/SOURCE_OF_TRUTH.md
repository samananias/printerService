...
- 🟢 **PIN login gate (2026-09-04, full decision record: [LOGIN_PLAN.md](LOGIN_PLAN.md)):** with no router-level device isolation available, the PIN got a real front door. When `API_PIN` is set, the web UI shows a one-time login gate (`POST /auth/login`) that exchanges the raw PIN for an opaque 256-bit session token — held only in server memory, so any restart invalidates every token, which (since config loads once at startup) also covers PIN changes with the same rule. State-changing routes accept either the raw `X-API-PIN` header (unchanged, additive) or the token in `X-Session-Token`. Read-only GETs stay open per the existing convention: the gate stops people from *acting*, not *reading* — an accepted property of the LAN threat model, recorded deliberately, not an oversight.
...
| `/auth/status` | GET | optional: stored session token in `X-Session-Token` | `{"pin_required": true/false, "session_valid": true/false/null}` | The login gate's pre-render check: does a gate exist, and is this device's remembered token still good? Always open, always 200 — same never-errors spirit as `/scanners`. |
| `/auth/login` | POST | `{"pin": "..."}` | `200 {"token": "..."}`; `401` wrong PIN; `400` no PIN configured | The only place the raw PIN is ever submitted; exchanges it once per device for an opaque session token (in-memory, restart-invalidated). |
