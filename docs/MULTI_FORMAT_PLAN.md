...
- [ ] Run T5/T6/T7 on the real print-server PC; record results in
      SOURCE_OF_TRUTH §5.
- [ ] Confirm the target PC's Windows version before Phase 3 (pin LO 7.6.x
      if Win 7/8.1).
- [ ] Phase 2: verify cancel cleanup covers non-PDF extensions.
- [ ] Phase 7: verify long-bond paper on the Epson driver (custom mm size
      vs driver paper name).
